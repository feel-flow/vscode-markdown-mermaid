...
- テスト: 未決定（Vitest または Mocha を検討中。拡張テストは @vscode/test-electron を予定）
...
## AI仕様駆動 Git Workflow（必須）
- **実装前に必ず feature ブランチを作成する。** develop/main で直接実装しない。ブランチ名: `feature/#{issue-number}-{description}`
- 作業は Issue から開始し、ブランチ作成 → 実装 → コミット → PR の流れで進める。
- コミットメッセージにドキュメント参照を含める（例: `docs/MASTER.md`）
- PR 作成前にセルフレビュー。マージ後にナレッジを記録。詳細は [AGENTS.md](../AGENTS.md) の「Git Workflow」を参照。
